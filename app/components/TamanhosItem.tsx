...
type TamanhosItemProps = {
    data: SizeType
}
const TamanhosItem = ({ data }: TamanhosItemProps) => {
    ...
    const teste = (id: string) => {
        ...
        // cancelModal.onOpen()
        if (id) {
            axios.delete(`/api/${params.storeId}/tamanhos/${id}`)
                .then(() => {
                    router.refresh()
                    ...
                    <div className='flex justify-center'>
                        {isOpen && (
                            <div ref={dropdownRef} className='absolute top-10 border rounded z-10 flex flex-col bg-white gap-2 p-4'>
                                ...
                                <div className='flex items-center hover:bg-slate-100 px-2 rounded cursor-pointer gap-2' onClick={() => router.push(`/${params.storeId}/tamanhos/${data.id}`)}>
                                    <GrUpdate />
                                    <p>Modificar</p>
                                </div>
